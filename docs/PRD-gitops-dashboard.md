...
| ----- | -------------------- | ------------------ |
| **Methodology POC** | Full OpenSpec lifecycle for argocd-connector `/healthz` + `/apps` endpoints: proposal generation, discipline reviews (6 reviewers), FastAPI implementation with httpx + Redis + structlog, TDD test suite, Docker build, post-implementation review. Validates scaffold is functional (`make check` passes). | Verify review findings are actionable. Confirm the methodology produces a working artifact. Decide if process adjustments are needed before committing to the full build. |
...
**The Methodology POC must pass its gate before any subsequent phase begins.** It validates that the OpenSpec lifecycle (propose → review → implement → review → archive), the discipline review process, the CI tooling, and the development workflow produce a working, tested, standards-compliant artifact. If the POC exposes fundamental issues with the methodology, the team adjusts before committing to the full 4-phase build.

| Phase | Scope | Deliverables | Gate Condition |
| ----- | ----- | ------------ | -------------- |
| **Methodology POC** (Week 0) | Prove the OpenSpec lifecycle works end-to-end by building a single narrow vertical slice: argocd-connector `/healthz` and `/apps` endpoints with mock ArgoCD responses, Redis caching, TDD tests, and a running Docker container. Exercises every layer (Python, FastAPI, Pydantic, httpx, Redis, structlog, Docker, pytest) and every lifecycle step (propose → discipline review → implement → test → post-impl review → archive). | A single argocd-connector container that: (1) serves `/healthz` and `/readyz` with real health checks, (2) serves `/apps` returning mock ArgoCD application data with correct Pydantic response models, (3) caches responses in Redis with 30-min TTL, (4) logs via structlog with scrub_secrets processor, (5) passes `make lint && make typecheck && make test` with 90%+ coverage, (6) builds and runs via `docker build && docker run`, (7) has been through both a pre-implementation and post-implementation discipline review with findings addressed. Also validates that the repo scaffold is functional: uv sync works, ruff passes, mypy passes, Docker builds succeed. | `make check` passes (lint + typecheck + test). Docker image builds and serves `/healthz` (200) and `/apps` (mock data). Pre-impl review findings addressed. Post-impl review findings addressed. All review documents archived. Scaffold confirmed functional. |
| **Phase 0** (Weeks 1–3) | Repo scaffold established (completed). Mock UI dashboard. | Repo scaffold (completed). Working dashboard mockup deployable as a Docker container (React static UI with mock data representing all 3 envs and 2 regions, no live connectors). Mockup is the visual contract for subsequent work. **Docs**: Project README.md, QUICKSTART.md (mock data mode), TROUBLESHOOTING.md (initial). | Mockup Docker image builds and runs locally. Stakeholder sign-off on env/region layout. README and QUICKSTART validated. |
...
| **Methodology POC before Phase 0** | A narrow vertical slice (argocd-connector `/healthz` + `/apps` with mock data, Redis, TDD tests, Docker) must pass the full OpenSpec lifecycle before any subsequent phase begins. This validates that the methodology (propose → discipline review → implement → test → post-impl review → archive) produces working, standards-compliant code. If the POC exposes fundamental issues, the team adjusts before committing to the full 4-phase build. See §12 Phased Rollout. |
