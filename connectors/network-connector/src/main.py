...
configure_logging(connector="network-connector")
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # TODO: Initialize Kubernetes API client, Redis connection pool
    import structlog
    log = structlog.get_logger()
    await log.ainfo("network-connector starting")
    yield
    await log.ainfo("network-connector shutting down")


app = FastAPI(
    title="Network Connector API",
    version="0.1.0",
    description="Queries Kubernetes API for NetworkPolicy resources across AKS clusters.",
    lifespan=lifespan,
)
...
async def liveness() -> HealthResponse:
    ...
    return HealthResponse(
        ...
        environment=os.environ.get("NETWORK_ENV", "unknown"),
        region=os.environ.get("NETWORK_REGION", "unknown"),
...
async def readiness() -> HealthResponse:
    ...
    # TODO: Actually check Kubernetes API and Redis connectivity
    return HealthResponse(
        status="ok",
        connector="network-connector",
        environment=os.environ.get("NETWORK_ENV", "unknown"),
